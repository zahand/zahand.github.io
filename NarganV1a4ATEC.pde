...
color CAT3 = color(255, 60, 0);

color BACKGROUND = color(55, 59, 68);

color WATER_FILL = color(35, 38, 47);

color NEIGH_STROKE = color(80, 83, 91);

color OST_FILL = color(92, 98, 112);
color OST_FILL_MIN = color(55, 59, 68);
color OST_FILL_MAX = color(92, 98, 112);
color OST_STROKE = color(142, 148, 167);
color OST_TEXT = color(255);

color CLUSTER_FILL = color(41, 44, 54, 180);
color CLUSTER_STROKE = color(255, 180);

color CLUSTER_PIE_FILL = color(41, 44, 54, 128);
color CLUSTER_PIE_TEXT = color(255, 180);

color OIL = color(71, 179, 165);
color GAS = color(140, 214, 0);
color PIPE = color(255, 140);
...
class Ost {
  ...
  void drw(boolean boo) {
    ...
    if (anim > 0) {
      cStroke(setAlpha(BACKGROUND, 150));
      cStrokeWeight(1);          
...
void selectProject(Project p) {
  document.getElementById("details").className = "hid";
...
void updateDetailsDiv() {
  ...
  d = document.getElementById("detailsss");
  d.innerHTML = 
  "<figure class='hide'><img src='http://lorempixel.com/400/300' width='100%'/></figure> <figure><img src='http://lorempixel.com/400/200' width='100%'/></figure><span class='prev'>«</span><span class='next'>»</span>";
  //2019-03-02 changed class='show' to class='hide' to temporarily get rid of the image slider ^^^
  ...
  detailsY = (height - dHeight + hh) / 2.0;

  window.setTimeout(updatescroll, 0);
  initSlideshow();

  sLoop();
  if (selP.N && selP.E) setMap(selP.N, selP.E, selP.z);
  else if (selP.cat == 0) setMap(-23.673494, 137.675686, 15);
  else if (selP.cat == 2) setMap(30.526861, 49.147497, 17);
  else if (selP.cat == 1) setMap(40.690067, -113.900362, 17);
  else if (selP.cat == 3) setMap(-19.705385, 121.255331, 17);
}
